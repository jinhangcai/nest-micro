...
export class ProdService {
  constructor(
    ...
    // private ProdRepository: Repository<Micro_search_routeEntity>,
    @InjectRepository(Micro_register_routeEntity)
    private usersRepository: Repository<Micro_register_routeEntity>,
    @InjectRepository(Micro_register_route_secondEntity)
    private usersSecondRepository: Repository<Micro_register_route_secondEntity>,
    private dataSource: DataSource,
    private readonly httpService: HttpService,
    private readonly mailerService: MailerService
  ) {}
  getHello(query: number, name: string) {
    return new Promise((resolve, reject) => {
      this.httpService.get('http://local.mistong.com:4000/prod/list').pipe(map(response => {
  ...
  getAxios(query, ssoHeader: SSOHeaderEntity) {
    console.log('ProdService', query)
    const url = '/api/ssoprod/permission/getMenuList'
    return new Promise((resolve, reject) => {
      this.httpService
        .get(url, {
          // params: {pageIndex: 1, pageSize: 10, sortOrder: 1},
          headers: { ...ssoHeader },
        }).pipe((
        map(response => {
  ...
  async getMysql() {
    // let newProd = new Micro_search_routeEntity();
    // newProd.baseName = 'novak';
    // newProd.micro_List = [{path:'abc'}]
    //  const data = await this.ProdRepository.find()


    // relations: ['routes']， 联合查询
    return await this.usersRepository.find({ relations: ['routes'], where:{id: 14} });

    // 查
    // return await this.ProdRepository.findAndCount()
    // return await this.ProdRepository.query('select * from prod_entity where id=2')
    // return await this.ProdRepository.find({where:{id:1},order:{name: 'ASC', id:'DESC'}})
    // 删
    // return await this.ProdRepository.delete(2)
    // 增 改
    // return await this.ProdRepository.save(newProd).then(res => {
    //   console.log('完毕回调', res)
