...
@Injectable()
export class Micro_SecondService {
  constructor(
    // 链接对象
    @InjectRepository(Micro_register_route_secondEntity)
    private usersRepository: Repository<Micro_register_route_secondEntity>,
    private dataSource: DataSource,
    private readonly httpService: HttpService,
  ) {}

  async createMicro({name, path, micro}) : Promise<Micro_register_route_secondEntity> {
    let newProdSecond = new Micro_register_route_secondEntity();
    newProdSecond.path = path;
    newProdSecond.name = name;
    newProdSecond.micro = micro;
    return await this.usersRepository.manager.save(newProdSecond)
  }

  async getMirco({name, id}) :Promise<Micro_register_route_secondEntity[]> {
    // 连表查询
    return await this.usersRepository.query('select a.name as prename, b.* from micro_register_route_entity a, micro_register_route_second_entity b where a.id=b.microId')
    // console.log('u', u);
    // return await this.usersRepository.find({where:{name: name || null, id: id || null}})
  }

  async delMicro({id}) : Promise<string> {
    console.log('getMirco', id);
    await this.usersRepository.delete(id)
    return '删除成功'
  }

  async updateMicro({ path, id, name, micro }) : Promise<boolean> {
    console.log('getMirco', id);
